...
void solve() {
    ...
    rep(i, 0, n) {
        ...
        int minOne_array = *min_element(all(v));
        pure_min = min(pure_min, minOne_array);
        v.erase(find(all(v), minOne_array));
        second_mins.push_back(*min_element(all(v)));
    }
    cout << pure_min + accumulate(all(second_mins), 0ll) - *min_element(all(second_mins)) << endl;
}
