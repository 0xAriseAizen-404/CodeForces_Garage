...
void solve() {
    ...
    bool has_divisible = false;

    rep(i, 0, n) {
        long long x; cin >> x;
        if (x % k == 0) has_divisible = true;
        // distance to next multiple of k
        ans = min(ans, (k - (x % k)) % k);
        if (k == 4 && (x & 1) == 0) cnt_even++;
    }

    if (k == 4) {
        // special rule for k=4: either we already have divisible by 4,
        // or we can rely on even-count trick, or add to next multiple.
        long long special = (n >= 2 ? max(0LL, 2LL - cnt_even) : (long long)4e18);
        long long res = min(ans, special);
        if (has_divisible) res = 0;
        cout << res << '\n';
    } else {
        cout << (has_divisible ? 0 : ans) << '\n';
    }
}
